...
public class Subway {
    public static void main(String[] args) throws IOException {
        if (args[0].equals("-map")) {
            new DataStruct(args[1]);
        } else if (args[0].equals("-a")) {
            if (args[2].equals("-map")) {
                new DataStruct(args[3]);
            } else return;
            List<Station> stations = Dijksta.getLineStation(args[1]);
            String content = "";
            if (stations == null)
                content = "不存在该条线路";
            else {
            ...
            if (args[3].equals("-map"))
                new DataStruct(args[4]);
            else return;
            Station star = null;
            Station end = null;
            for (List<Station> l : DataStruct.lineSet) {
            ...
            if (star == null || end == null)
                ...
                Result r = Dijksta.shortestPath(star, end);
                List<String> path = Dijksta.getPath(r);
